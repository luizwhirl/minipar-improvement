...
int main() {
    ...
    auto arr = std::vector<double>{33, 12, 98, 5, 61};
    auto start_stack = std::vector<double>{0};
    auto end_stack = std::vector<double>{4};
    std::cout << "Array original: " << arr << std::endl;
    while ((start_stack.size() > 0))
    {
        auto start = __array_pop(start_stack);
        auto end = __array_pop(end_stack);
        if ((start < end))
        {
            auto pivot = arr[end];
            auto i = (start - 1);
            ...
            end_stack.push_back((pi - 1));
            start_stack.push_back((pi + 1));
            end_stack.push_back(end);
        }
    }
    std::cout << "Array ordenado: " << arr << std::endl;
