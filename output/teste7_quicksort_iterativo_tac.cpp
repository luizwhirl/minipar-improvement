...
auto quicksort_iterativo(auto arr) {
    auto stack = std::vector<double>{0, 4};
    auto sp = 2;
    while ((sp > 0))
    {
        sp = (sp - 1);
...
int main() {
    ...
    auto array = std::vector<double>{33, 12, 98, 5, 61};
    auto ordenado = quicksort_iterativo(array);
    std::cout << "Array ordenado:" << " " << ordenado << std::endl;
