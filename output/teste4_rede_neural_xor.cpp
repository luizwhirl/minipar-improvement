...
MiniParAny sigmoid_deriv(MiniParAny x) {
    return (x * (1 - x));
}
...
public:
    decltype(0) p0 = 0;
    decltype(0) p1 = 0;
    decltype(0) bias_val = 0;
    decltype(0) saida = 0;
    void init(MiniParAny w0, MiniParAny w1, MiniParAny b) {
        this->p0 = w0;
        this->p1 = w1;
        this->bias_val = b;
        this->saida = 0;
    }
    MiniParAny feedforward2(MiniParAny e0, MiniParAny e1) {
        auto soma = (((e0 * this->p0) + (e1 * this->p1)) + this->bias_val);
        this->saida = sigmoid(soma);
        return this->saida;
    }
    MiniParAny feedforward3(MiniParAny e0, MiniParAny e1, MiniParAny e2) {
        auto soma = (((e0 * this->p0) + (e1 * this->p1)) + (e2 * this->bias_val));
        this->saida = sigmoid(soma);
        return this->saida;
    }
};
...
class RedeNeural {
public:
    decltype(0.2) taxa = 0.2;
    decltype(0) n0 = 0;
    decltype(0) n1 = 0;
    decltype(0) n2 = 0;
    decltype(0) ns = 0;
    void inicializar() {
        this->n0 = std::make_shared<Neuronio>();
        this->n1 = std::make_shared<Neuronio>();
        this->n2 = std::make_shared<Neuronio>();
        this->ns = std::make_shared<Neuronio>();
        this->n0->init(0.5, 0.5, 0.5);
        this->n1->init(0.5, 0.5, 0.5);
        this->n2->init(0.5, 0.5, 0.5);
        this->ns->init(0.5, 0.5, 0.5);
    }
    MiniParAny forward(MiniParAny e0, MiniParAny e1) {
        auto o0 = this->n0->feedforward2(e0, e1);
        auto o1 = this->n1->feedforward2(e0, e1);
        auto o2 = this->n2->feedforward2(e0, e1);
        auto out = this->ns->feedforward3(o0, o1, o2);
        return out;
    }
    void treinar_amostra(MiniParAny e0, MiniParAny e1, MiniParAny alvo) {
        auto o0 = this->n0->feedforward2(e0, e1);
        auto o1 = this->n1->feedforward2(e0, e1);
        auto o2 = this->n2->feedforward2(e0, e1);
        auto out = this->ns->feedforward3(o0, o1, o2);
        auto erro = (alvo - out);
        auto delta_s = (erro * sigmoid_deriv(out));
        ...
        auto d0 = ((delta_s * this->ns->p0) * sigmoid_deriv(o0));
        auto d1 = ((delta_s * this->ns->p1) * sigmoid_deriv(o1));
        auto d2 = ((delta_s * this->ns->bias_val) * sigmoid_deriv(o2));
    ...
    void treinar_epocas(MiniParAny epocas) {
        auto ep = 0;
        while ((ep < epocas))
        {
...
int main() {
    ...
    std::cout << "=== Rede Neural XOR - MiniPar 2026.1 ===" << std::endl;
    std::cout << "--- Treinando Rede Neural XOR ---" << std::endl;
    std::cout << "Epocas: 20000" << std::endl;
    auto rede = std::make_shared<RedeNeural>();
    rede->inicializar();
    rede->treinar_epocas(20000);
    std::cout << "Treinamento concluido" << std::endl;
    std::cout << "--- Testando Rede Neural ---" << std::endl;
    auto r00 = rede->forward(0, 0);
    std::cout << "Input: [0, 0], Predicted Output:" << " " << r00 << std::endl;
    auto r01 = rede->forward(0, 1);
    std::cout << "Input: [0, 1], Predicted Output:" << " " << r01 << std::endl;
    auto r10 = rede->forward(1, 0);
    std::cout << "Input: [1, 0], Predicted Output:" << " " << r10 << std::endl;
    auto r11 = rede->forward(1, 1);
    std::cout << "Input: [1, 1], Predicted Output:" << " " << r11 << std::endl;
