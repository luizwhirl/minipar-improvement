...
auto sigmoid(auto x) {
    return (1.0 / (1.0 + std::exp((0.0 - x))));
}

class NeuronioXOR {
public:
    decltype(0.6) peso1 = 0.6;
    decltype(0.4) peso2 = 0.4;
    decltype(0.2) bias = 0.2;
    decltype(0.0) saida = 0.0;
    auto feedforward(auto e1, auto e2) {
        auto soma = (((e1 * this->peso1) + (e2 * this->peso2)) + this->bias);
...
int main() {
    ...
    auto n = std::make_shared<NeuronioXOR>();
    std::cout << "Testando XOR (0, 0) Predicted Output: " << n->feedforward(0, 0) << std::endl;
    std::cout << "Testando XOR (0, 1) Predicted Output: " << n->feedforward(0, 1) << std::endl;
    std::cout << "Testando XOR (1, 0) Predicted Output: " << n->feedforward(1, 0) << std::endl;
    std::cout << "Testando XOR (1, 1) Predicted Output: " << n->feedforward(1, 1) << std::endl;
