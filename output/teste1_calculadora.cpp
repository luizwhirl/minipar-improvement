...
int main() {
    ...
    auto calculadora = std::make_shared<MiniParChannel>("127.0.0.1", 8080);
    {
        std::vector<std::thread> __m_thr;
        __m_thr.emplace_back([&]() {
            {
                auto req = calculadora->receiveData();
                std::cout << "Servidor recebeu a operacao: " << req << std::endl;
                calculadora->sendData(__to_string("Resultado da operacao = 15"));
            }
        });
        __m_thr.emplace_back([&]() {
            {
                ...
                std::cout << "Enviando solicitacao (10 + 5)..." << std::endl;
                calculadora->sendData(__to_string("10 + 5"));
                auto res = calculadora->receiveData();
                std::cout << "Cliente recebeu: " << res << std::endl;
            }
        });
