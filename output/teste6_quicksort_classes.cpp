...
class Quicksort {
public:
    MiniParAny array = std::vector<double>{10, (-3), (-40), 80, 70, (-100)};
    void trocar(MiniParAny i, MiniParAny j) {
        auto temp = this->array[(int)i];
        this->array[(int)i] = this->array[(int)j];
        this->array[(int)j] = temp;
    }
    MiniParAny particionar(MiniParAny inicio, MiniParAny fim) {
        auto pivot = this->array[(int)fim];
        auto i = (inicio - 1);
        auto j = inicio;
        while ((j < fim))
        {
            if ((this->array[(int)j] <= pivot))
            {
                i = (i + 1);
                this->trocar(i, j);
            }
    ...
    void quicksort(MiniParAny inicio, MiniParAny fim) {
        if ((inicio < fim))
        {
            auto p = this->particionar(inicio, fim);
...
int main() {
    ...
    std::cout << "==== Ordenação com Quicksort ====" << std::endl;
    std::cout << "Insira os elementos do vetor separados por espaço:" << std::endl;
    std::cout << "10 -3 -40 80 70 -100" << std::endl;
    auto algoritmo = std::make_shared<Quicksort>();
    std::cout << "Vetor original: [10, -3, -40, 80, 70, -100]" << std::endl;
    auto vetor_ordenado = algoritmo->ordenar();
    std::cout << "Vetor ordenado:" << " " << vetor_ordenado << std::endl;
