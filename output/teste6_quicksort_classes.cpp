...
class Quicksort {
public:
    decltype(std::vector<double>{10, (-3), (-40), 80, 70, (-100)}) array = std::vector<double>{10, (-3), (-40), 80, 70, (-100)};
    void trocar(auto i, auto j) {
        auto temp = this->array[i];
        this->array[i] = this->array[j];
        this->array[j] = temp;
    }
    auto particionar(auto inicio, auto fim) {
        auto pivot = this->array[fim];
        auto i = (inicio - 1);
        auto j = inicio;
        while ((j < fim))
        {
    ...
    void quicksort(auto inicio, auto fim) {
        if ((inicio < fim))
        {
