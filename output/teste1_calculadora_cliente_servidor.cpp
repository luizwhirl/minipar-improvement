...
int main() {
    ...
    std::cout << "==== Calculadora cliente-servidor ====" << std::endl;
    auto requisicao = std::make_shared<MiniParChannel>("127.0.0.1", 5101);
    auto resposta = std::make_shared<MiniParChannel>("127.0.0.1", 5102);
    {
        std::vector<std::thread> __m_thr;
        __m_thr.emplace_back([&]() {
            {
                auto pedido = requisicao->receiveData();
                std::cout << "Servidor recebeu:" << " " << pedido << std::endl;
                resposta->sendData(__to_string("Resultado: 12"));
            }
        });
        __m_thr.emplace_back([&]() {
            {
                requisicao->sendData(__to_string("+ 7 5"));
                auto resultado = resposta->receiveData();
